...
(function () {
  ...
  function renderDataTab() {
    ...
    /* ---- 数据来源横幅 + 导入工具 ---- */
    html += renderSourceBar();

    /* ---- 实时抓取数据（油猴脚本注入） ---- */
    if (liveData) {
      html += renderLiveSections(liveData);
      html += '<div style="padding:10px 14px;background:var(--beige);border:1px dashed var(--line);border-radius:8px;font-size:11.5px;color:var(--text-3);margin-bottom:20px">' +
        '👇 以下为<b>静态示例数据</b>（2026-08-09 后台截图），用于演示分析方法；上方为实时抓取的真实数据。</div>';
    }
    ...
    html += '<div style="font-size:12px;color:var(--text-3);margin-top:4px">' +
      (liveData ? '数据来源：实时抓取 · ' + (liveData.capturedAt || '') + '（静态示例见下）'
                : '数据来源：2026-08-09 后台截图 · 指导投流、定价、内容、达人合作') + '</div>';
    html += '</div>';
  ...
  function loadLive() {
    try { liveData = JSON.parse(localStorage.getItem(LIVE_KEY) || 'null'); } catch (e) { liveData = null; }
  }

  function handleLivePayload(p) {
    if (!p || !Array.isArray(p.blobs)) return false;
    liveData = p;
    try { localStorage.setItem(LIVE_KEY, JSON.stringify(p)); } catch (e) { /* 超出配额忽略 */ }
    renderDataTab();
    return true;
  }
  ...
  /* ---- 实时数据区块渲染 ---- */
  function renderLiveSections(p) {
    var html = '';

    /* 核心指标卡 */
    if (p.metrics && p.metrics.length) {
      html += '<div class="shot" style="margin-bottom:20px"><div class="shot-title">实时核心指标（从接口自动提取）</div>';
      html += '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px">';
      p.metrics.forEach(function (m) {
        var v = typeof m.value === 'number' && m.value >= 10000 ? (m.value / 10000).toFixed(2) + '万' : m.value;
        html += '<div style="padding:10px 12px;background:var(--bg);border-radius:8px;border:1px solid var(--border)">' +
          '<div style="font-size:11px;color:var(--text-3);margin-bottom:2px">' + escHtml(m.key) + '</div>' +
          '<div style="font-size:17px;font-weight:700;color:var(--pri)">' + escHtml(v) + '</div>' +
          '<div style="font-size:10px;color:var(--text-3);margin-top:2px">' + escHtml(m.source) + '</div></div>';
      });
      html += '</div></div>';
    }
    ...
    if (p.tables && p.tables.length) {
      html += '<div class="shot" style="margin-bottom:20px"><div class="shot-title">页面表格（' + p.tables.length + ' 个）</div>';
      p.tables.forEach(function (t, i) {
        html += '<details style="margin-bottom:8px"><summary style="font-size:12.5px;color:var(--text-2);cursor:pointer">' +
          escHtml(t.title || ('表格 ' + (i + 1))) + '（' + (t.rows || []).length + ' 行）</summary>';
        html += '<table class="shots"><thead><tr>' + (t.headers || []).map(function (h) { return '<th>' + escHtml(h) + '</th>'; }).join('') + '</tr></thead><tbody>';
    ...
    if (p.blobs && p.blobs.length) {
      html += '<div class="shot" style="margin-bottom:20px"><div class="shot-title">抓取的数据接口（' + p.blobs.length + ' 个，最新在前）</div>';
      p.blobs.slice(-10).reverse().forEach(function (b) {
        var pretty;
        try { pretty = JSON.stringify(b.data, null, 2); } catch (e) { pretty = String(b.data); }
        html += '<details style="margin-bottom:8px"><summary style="font-size:12px;color:var(--text-2);cursor:pointer;word-break:break-all">' +
          escHtml(b.url) + ' · ' + escHtml(b.time || '') + '</summary>';
        html += '<pre style="max-height:320px;overflow:auto;font-size:11px;background:var(--bg);border:1px solid var(--border);border-radius:7px;padding:10px;white-space:pre-wrap;word-break:break-all">' +
          escHtml(pretty) + '</pre></details>';
      });
      html += '</div>';
    }
    ...
    if (!p.metrics || !p.metrics.length) {
      if (!p.tables || !p.tables.length) {
        if (!p.blobs || !p.blobs.length) {
          html += '<div class="shot" style="margin-bottom:20px"><div class="shot-title">实时数据</div>' +
            '<p class="hint" style="padding:8px 0">采集到 0 条数据。请在抖音来客后台打开「经营概览 / 数据报表 / 投放分析」等页面后重新采集。</p></div>';
        }
      }
    }
    return html;
  ...
  /* ---- URL 参数/哈希兜底：#data= 或 ?data=（base64url） ---- */
  ...
  function parseInjected() {
    var m = location.hash.match(/data=([A-Za-z0-9_\-]+)/) ||
            location.search.match(/[?&]data=([A-Za-z0-9_\-]+)/);
    if (!m) return;
    try {
      var p = JSON.parse(b64ToUtf8(m[1]));
      if (handleLivePayload(p)) toast('已通过链接导入抓取数据');
    } catch (e) { /* 非法数据忽略 */ }
  }
  /* 页面已打开时用户再粘贴带 #data= 的链接：监听 hashchange 免刷新导入 */
  window.addEventListener('hashchange', function () { parseInjected(); });
